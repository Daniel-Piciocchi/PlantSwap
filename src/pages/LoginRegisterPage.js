...
const LoginRegisterPage = () => {
  ...
  const registerFormik = useFormik({
    ...
    onSubmit: async (values) => {
      console.log(values);
      const response = await fetch('http://plantswap-6dabb95ad1f6.herokuapp.com/auth/register', {
        method: 'POST',
        headers: {
  ...
  const loginFormik = useFormik({
    ...
    onSubmit: async (values) => {
      console.log(values);
      const response = await fetch('http://plantswap-6dabb95ad1f6.herokuapp.com/auth/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
